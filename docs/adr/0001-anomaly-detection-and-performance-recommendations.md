...
Severity (`warning` vs `critical`) is graded from a z-score against the baseline's own mean/std, not from `IsolationForest`'s `decision_function` score directly — discovered while writing the unit tests: with a single (univariate) feature, the score saturates once a point falls outside the training range, since every split in every tree routes it the same way regardless of *how far* outside it is. A barely-outlying point and a wildly-outlying point ended up with the identical score in testing. The z-score against the baseline's own distribution doesn't have that ceiling, so it's what severity is graded on; `decision_function`'s score is still stored (`anomalies.anomaly_score`) and used for detection (`predict() == -1`), just not for severity.
